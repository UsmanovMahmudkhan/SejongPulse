...
export default function Navbar() {
  ...
  useEffect(() => {
    // Left empty since unreadCount logic is moving to Header
  }, [pathname]);

  const navItems = [
    { href: "/pulse", icon: "amp_stories", label: "Feed" },
    { href: "/advisor", icon: "auto_awesome", label: "Advisor" },
    { href: "/messages/chats", icon: "chat", label: "Chat" },
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 pb-6 pt-3 bg-white/60 backdrop-blur-[40px] shadow-[0_-10px_30px_rgba(0,0,0,0.05)] rounded-t-[2rem] border-t border-outline-variant/20">
      {navItems.map((item) => {
        const isActive = pathname === item.href || (item.href !== "/pulse" && pathname.startsWith(item.href));
        return (
          <Link
            key={item.href}
            href={item.href}
            className={`flex flex-col items-center justify-center p-3 transition-all duration-300 hover:scale-110 relative
              ${isActive
          ...
          >
            ...
            {item.badge && item.badge > 0 && !isActive && (
              <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-primary text-white text-[9px] font-black flex items-center justify-center shadow-sm">
